"use client";
import { convertType, getAllUsers, getUser } from "@/app/auth";
import {
  Box,
  List,
  ListItem,
  ...
  return (
    ...
    >
      <Dialog open={dialog} onClose={() => {}} fullWidth>
        <DialogTitle>
          {`Change Type of (Phone:${selectedUser.phone_number}, Name: ${selectedUser.user_name})`}
        </DialogTitle>
        <DialogContent>
          <Box
            sx={{
          ...
          {dialogMsg == "" ? (
            <Button
              onClick={() => {
                ...
                convertType(selectedUser.phone_number, makeAdmin).then(
                  (result) => {
                    if (result.status === true) {
                      setDialogMsg(
                        `Phone:${selectedUser.phone_number}, Name: ${
                          selectedUser.user_name
                        } Is Changed To ${
                          makeAdmin == true ? "Admin" : "User"
                        } Successfully`
                      );
                    } else if (
                      result.error &&
                      result.error.includes("can not change yourself")
                    ) {
                      setDialogMsg("You Can Not Change Your Own Account");
                    } else {
                      setDialogMsg(
                        "Failed To Change Type Because Of Unknown Reason"
                      );
                    }
